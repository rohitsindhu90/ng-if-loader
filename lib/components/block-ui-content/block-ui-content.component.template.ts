...
<div class="block-ui-wrapper {{name}} {{className}} {{customClass}}" [ngClass]="{ 'active': state.blockCount > 0,'in-active':state.blockCount==0 }" [ngStyle]="{'min-height': (height && state.blockCount > 0?height:'auto'),'min-width': (width && state.blockCount > 0?width:'auto')}">
